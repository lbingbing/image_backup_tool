    ...
    if (m_running) {
        ...
        try {
            ...
            while (done1 < sizeof(uint64_t)) {
                done1 += boost::asio::read(socket, boost::asio::buffer(reinterpret_cast<char*>(&task_status_byte_len) + done1, sizeof(uint64_t) - done1));
            }
            if (task_status_byte_len) {
                Bytes task_status_bytes(task_status_byte_len);
                uint64_t done2 = 0;
                while (done2 < task_status_byte_len) {
                    done2 += boost::asio::read(socket, boost::asio::buffer(task_status_bytes.data() + done2, task_status_byte_len - done2));
                }
            }
        }
...
void TaskStatusServer::Worker() {
    while (m_running) {
        try {
            ...
            uint64_t done1 = 0;
            while (done1 < sizeof(uint64_t)) {
                done1 += boost::asio::write(socket, boost::asio::buffer(reinterpret_cast<char*>(&task_status_byte_len) + done1, sizeof(uint64_t) - done1));
            }
            if (task_status_byte_len) {
                uint64_t done2 = 0;
                while (done2 < task_status_byte_len) {
