...
void start(const std::string& output_file, const Dim& dim, PixelType pixel_type, int pixel_size, int space_size, uint32_t part_num, int mp, const Transform& transform, const Calibration& calibration) {
    ImageDecodeWorker image_decode_worker(pixel_type);
    ...
    std::thread fetch_image_thread(&ImageDecodeWorker::FetchImageWorker, &image_decode_worker, std::ref(running), std::ref(frame_q), 20);
    auto get_transform_fn = [transform] {
        return transform;
    };
    std::vector<std::thread> decode_image_threads;
    for (int i = 0; i < mp; ++i) {
        decode_image_threads.emplace_back(&ImageDecodeWorker::DecodeImageWorker, &image_decode_worker, std::ref(part_q), std::ref(frame_q), dim, get_transform_fn, calibration);
    }
    ...
    std::thread decode_image_result_thread(&ImageDecodeWorker::DecodeResultWorker, &image_decode_worker, std::ref(part_q), std::ref(frame_q), dim, get_transform_fn, calibration, send_decode_image_result_cb, 500);
    std::thread show_image_thread(show_image_worker, std::ref(running), std::ref(show_image_q));
    auto save_part_progress_cb = [](const ImageDecodeWorker::SavePartProgress& task_save_part_progress){
